...
savetask.addEventListener("click", () => {
  taskList[edittask.dataset.index] = new Task(
    projectname.value,
    taskinfo.value,
    taskdate.value
  );
  ...
  if (
    projectname.value !== edittask.dataset.project &&
    edittask.dataset.project !== ""
  ) {
...
function loadLocalStorage() {
  ...
  while (localStorage.getItem("task" + i + "%checked")) {
    let task = new Task(
      ...
      localStorage.getItem("task" + i + "%info"),
      localStorage.getItem("task" + i + "%duedate"),
      localStorage.getItem("task" + i + "%checked") === "true"
    );
    taskList.push(task);
    list.append(makeTaskObject(task));
    i++;
